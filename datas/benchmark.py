...
class Benchmark(data.Dataset):
    def __init__(self, HR_folder, LR_folder, scale=2, colors=1):
        ...
        for tag in tags:
            ...
            lr_filename = os.path.join(self.LR_folder, 'X{}'.format(scale),
                                       tag.replace('.png', 'x{}.png'.format(self.scale)))
            self.hr_filenames.append(hr_filename)
            self.lr_filenames.append(lr_filename)
            self.image_names.append(tag)
        ...
        for i in range(LEN):
            lr_image, hr_image = imageio.imread(self.lr_filenames[i], pilmode="RGB"), imageio.imread(
                self.hr_filenames[i], pilmode="RGB")
            if self.colors == 1:
    ...
    def __getitem__(self, idx):
        ...
        return lr, hr, self.image_names[idx]


if __name__ == '__main__':
    HR_folder = '../SRData/benchmark/B100/HR'
    LR_folder = '../SRData/benchmark/B100/LR_bicubic'
